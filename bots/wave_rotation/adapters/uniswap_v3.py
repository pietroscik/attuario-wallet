...
"""Adapter for Uniswap v3 concentrated liquidity pools."""
...
class UniswapV3Adapter(Adapter):
    """Adapter for Uniswap v3 concentrated liquidity positions."""
    ...
    def __init__(self, w3: Web3, config: Dict[str, object], signer, sender: str):
        ...
        try:
            ...
            self.fee = int(config.get("fee", 3000))  # default 0.3%
            self.tick_lower = int(config.get("tick_lower", -887220))
            self.tick_upper = int(config.get("tick_upper", 887220))
        except KeyError as exc:
            raise RuntimeError(f"[uniswap_v3] missing configuration field: {exc.args[0]}")

        self.nft_manager = _contract(w3, self.nft_manager_address, UNISWAP_V3_NFT_MANAGER_ABI)
    ...
    def withdraw_all(self) -> Dict[str, object]:
        # Note: Uniswap v3 positions are NFTs, so withdrawing requires knowing the tokenId
        # This is a simplified implementation that assumes no active positions
        return {
            "status": "not_implemented",
            "reason": "Uniswap v3 withdrawal requires NFT position management - implement position tracking",
        }
