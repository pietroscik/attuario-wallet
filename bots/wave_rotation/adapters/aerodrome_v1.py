...
"""Adapter for Aerodrome v1 LP pools (standard AMM, not concentrated liquidity)."""
...
def _load_config(raw: Dict[str, object]) -> AerodromeV1Config:
    try:
        router = raw["router"]
        ...
        raise RuntimeError(f"[aerodrome_v1] missing configuration field: {exc.args[0]}")

    return AerodromeV1Config(
        router=router,
        lp_token=lp_token,
        token0=token0,
        token1=token1,
...
class AerodromeV1Adapter(Adapter):
    """Adapter that routes liquidity to Aerodrome v1 LP pools (standard AMM)."""
    ...
    def __init__(self, w3: Web3, config: Dict[str, object], signer, sender: str):
        ...
        self.router = _contract(w3, self.cfg.router, AERODROME_ROUTER_ABI)
