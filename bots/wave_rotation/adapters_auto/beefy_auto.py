...
"""Auto adapter for Beefy-like vaults."""
...
from abi_auto import BEEFY_ABI, ERC20_ABI
...
class BeefyAuto:
    ...
    def probe(w3: Web3, address: str) -> bool:
        try:
            vault = w3.eth.contract(address=Web3.to_checksum_address(address), abi=BEEFY_ABI)
            _ = vault.functions.want().call()
            return True
        except Exception:
            return False
    ...
    def __init__(self, w3: Web3, signer, sender: str, address: str):
        ...
        self.vault = w3.eth.contract(address=Web3.to_checksum_address(address), abi=BEEFY_ABI)
        asset_address = self.vault.functions.want().call()
