...
"""Auto adapter for Sonne Finance (Compound V2 fork) cTokens."""
...
class SonneAuto:
    ...
    def probe(w3: Web3, address: str) -> bool:
        """Probe if address is a Sonne Finance cToken."""
        try:
            token = w3.eth.contract(address=Web3.to_checksum_address(address), abi=CTOKEN_ABI)
            _ = token.functions.underlying().call()
            # Check for Comptroller to verify it's a Compound V2 fork
            _ = token.functions.comptroller().call()
            return True
        except Exception:
            return False
